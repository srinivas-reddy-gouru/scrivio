...
def scrub_dashes_in_model(model):
    """Strip em and en dashes from every string a pydantic model carries.

    The product's rule is that nothing it writes contains these marks, and
    the rule was already enforced on resumes and articles. Interview text
    was generated by three other prompts that never mentioned it, so a
    question or a follow-up would arrive with an em dash in it. Prompt
    text alone has been measured failing here often enough that the rule
    gets a deterministic pass instead."""
    for name in type(model).model_fields:
        value = getattr(model, name, None)
        if isinstance(value, str):
            setattr(model, name, scrub_em_dashes(value))
        elif isinstance(value, list) and value and isinstance(value[0], str):
            setattr(model, name, [scrub_em_dashes(v) for v in value])
        elif hasattr(value, "model_fields"):
            scrub_dashes_in_model(value)
        elif isinstance(value, list):
            for item in value:
                if hasattr(item, "model_fields"):
                    scrub_dashes_in_model(item)
    return model
